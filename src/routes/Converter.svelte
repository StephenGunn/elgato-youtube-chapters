<script lang="ts">
	...
	const processContent = () => {
		convertedChapters = '';
		const chapters: Array<{ hours: number; minutes: number; seconds: number; title: string }> = [];
		...
		const offsetChapters = chapters.map((chapter) => {
			const adjusted = applyOffset(chapter.hours, chapter.minutes, chapter.seconds, offsetSeconds);
			return { ...chapter, ...adjusted };
		});

		// Check if the first timestamp is within 10 seconds
		const firstChapterTime =
			offsetChapters[0].hours * 3600 + offsetChapters[0].minutes * 60 + offsetChapters[0].seconds;
		...
		offsetChapters.forEach((chapter, index) => {
			...
			if ((firstChapterTime > 10 && index === 0) || (firstChapterTime <= 10 && index === 0)) {
